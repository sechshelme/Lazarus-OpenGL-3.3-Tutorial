...
const
  ...
  LightingShader =
    ...
    '  vec4 emissiv = Memission;' + LineEnding +
  ...
  {$else}
  LightingShader =
    'uniform vec4 Memission;' + LineEnding +
  ...
  for i := 0 to Length(MaterialPara) - 1 do begin
    ...
    if Name = MaterialPara[i].Name then begin
      with MaterialParams do begin
        emission := MaterialPara[i].emission;
...
begin
  if Lighting.Enabled then begin
    ...
    with Lighting.UniformID.Material do begin
      glUniform4fv(emission, 1, @MaterialParams.emission);
