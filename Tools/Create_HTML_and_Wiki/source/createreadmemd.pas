...
constructor TCreateReadmeMD.Create(const Datei, Kapitel: string);
begin
  inherited Create;
  fFolder := Datei;
  fKapitel := Kapitel;
...
destructor TCreateReadmeMD.Destroy;
var
  sl2: TStringList;
begin
...
function TCreateReadmeMD.TagResWort(const s: string; const ResWort: string): string;
var
...
function TCreateReadmeMD.TagNumberAndStringPascal(const s: string): string;
var
  p, ComentPos: integer;   // Position Kommentar
  isString,             // String wird getagt
  isComment,            // { und }
...
procedure TCreateReadmeMD.TagPascal(CodeSL: TStrings);
var
  i, p, j: integer;
  s: string;
begin
...
procedure TCreateReadmeMD.TagGLSL(glslCodeSL: TStrings);
var
  i, p, j: integer;
  s: string;
begin
...
procedure TCreateReadmeMD.AddPascalCode(datei: string);
var
  sl: TStringList;
  p: integer;
  block: string;
...
procedure TCreateReadmeMD.AddTextCode(const datei: string);
var
  sl: TStringList;
begin
  sl := TStringList.Create;
...
procedure TCreateReadmeMD.AddGLSLCode(const datei: string);
var
  slGLSL: TStringList;
begin
...
procedure TCreateReadmeMD.AddSouceUnit1(const datei: string);
var
  slUnit: TStringList;
  i, p: integer;
...
begin
  ...
  for i := 0 to slUnit.Count - 1 do begin
    if Pos('//includeglsl ', slUnit[i]) > 0 then begin
      ...
      isCode := False;
      TagPascal(slCode);
      slHTML.Add('<pre><code=pascal>' + slCode.Text + '</code></pre>');
