...
type
  ...
  public
    ...
    procedure AddCPPCode(const datei: string);
  end;
...
procedure TCreateWikiTutorial.AddCPPCode(const datei: string);
var
  slCPP: TStringList;
begin
  slCPP := TStringList.Create;
  slCPP.LoadFromFile(datei);
  slCPP.SkipLastLineBreak := False;

  slWiki.Add('<syntaxhighlight lang="cpp">' + slCPP.Text + '</syntaxhighlight>');
...
begin
  ...
  for i := 0 to slUnit.Count - 1 do begin
    if Pos('//includeglsl ', slUnit[i]) > 0 then begin
      ...
      AddCPPCode(TutPara.TutPfad + fFolder + '/' + Copy(slUnit[i], 14));
      ...
      // GLSL Code
    end else if Pos('//codeglsl+', slUnit[i]) > 0 then begin
      isCode := True;
      slCode.Clear;
    end else if Pos('//codeglsl-', slUnit[i]) > 0 then begin
      isCode := False;
      slWiki.Add('<syntaxhighlight lang="glsl">' + slCode.Text + '</syntaxhighlight>');
