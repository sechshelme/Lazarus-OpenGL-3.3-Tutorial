...
rtl.module("program",["System","JS","Web","browserconsole"],function () {
  "use strict";
  var $mod = this;
  this.ButtonClick = function (aEvent) {
    var Result = false;
    var xhr = null;
    function LocalLoad(aEvent) {
      var s = "";
      if (xhr.status === 200) {
        s = xhr.responseText;
    ...
    xhr.addEventListener("load",rtl.createSafeCallback(null,LocalLoad));
    xhr.open("GET","project1.lpr");
    xhr.responseType = "tedddxt";
    xhr.send();
  ...
  this.Button = null;
  $mod.$main = function () {
    $mod.Button = document.createElement("input");
    $mod.Button.setAttribute("id","Button");
    $mod.Button.setAttribute("type","submit");
    $mod.Button.className = "btn btn-default";
    $mod.Button.setAttribute("value","Lade Datei");
    $mod.Button.onclick = rtl.createSafeCallback($mod,"ButtonClick");
    document.body.appendChild($mod.Button);
