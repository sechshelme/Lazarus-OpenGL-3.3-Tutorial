...
begin
  ...
  Shader.Left := TShader.Create([FileToStr('Vertexshader.glsl'), FileToStr('Fragmentshader.glsl')]);
  //remove-
  with Shader.Left do begin
    UseProgram;
    MatrixLeft_ID := UniformLocation('Matrix');
    ModelMatrixLeft_ID := UniformLocation('ModelMatrix');

    Material_ID := UniformBlockIndex('Material'); // ID aus dem Shader holen.
  end;

  Shader.Right := TShader.Create([FileToStr('Vertexshader.glsl'), FileToStr('Fragmentshader.glsl')]);
  with Shader.Right do begin
    UseProgram;
    MatrixRight_ID := UniformLocation('Matrix');
    ModelMatrixRight_ID := UniformLocation('ModelMatrix');
...
begin
  ...
  glUniformBlockBinding(Shader.Left.ID, Material_ID, bindingPoint);
  glUniformBlockBinding(Shader.Right.ID, Material_ID, bindingPoint);
  ...
  for x := -CubeSize to CubeSize do begin
    for y := -CubeSize to CubeSize do begin
      for z := -CubeSize to CubeSize do begin
        ...
        Matrix.Uniform(ModelMatrixRight_ID);

        Matrix.Multiply(WorldMatrix, Matrix);                    // Matrixen multiplizieren.
        Matrix.Multiply(FrustumMatrix, Matrix);

        Matrix.Uniform(MatrixRight_ID);                               // Matrix dem Shader übergeben.
        glDrawArrays(GL_TRIANGLES, 0, Length(SphereVertex) * 3); // Zeichnet einen kleinen Würfel.
      end;
    end;
  end;
