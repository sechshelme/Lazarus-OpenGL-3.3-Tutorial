...
var
  mouse_ID, time_ID, Resolution_ID: GLint;    // ID für Matrix.
...
begin
  ...
  mouse_ID := Shader.UniformLocation('mouse');
...
begin
  ...
  r[0] := ClientWidth;
  r[1] := ClientHeight;
  m[0] := (Mouse.CursorPos.x - Left) / Width;
  m[1] := (Mouse.CursorPos.y - Top) / Height;



  glUniform1f(time_ID, zeit);
  glUniform2fv(Resolution_ID, 1, @r);
  glUniform2fv(mouse_ID, 1, @m);
